...
public class GamesTableController implements Controller<Node> {
  ...
  private final ChangeListener<Boolean> modChangedListener;
  private final ChangeListener<Boolean> passwordChangeListener;
  ...
  public GamesTableController(MapService mapService, JoinGameHelper joinGameHelper, I18n i18n, UiService uiService, PreferencesService preferencesService) {
    ...
    modChangedListener = (observable, oldValue, newValue) -> modsColumn.setVisible(newValue);
    passwordChangeListener = (observable, oldValue, newValue) -> passwordProtectionColumn.setVisible(newValue);
  }
  ...
  public void initializeGameTable(ObservableList<Game> games) {
    ...
    JavaFxUtil.addListener(preferencesService.getPreferences().showModdedGamesProperty(), new WeakChangeListener<>(modChangedListener));
    JavaFxUtil.addListener(preferencesService.getPreferences().showPasswordProtectedGamesProperty(), new WeakChangeListener<>(passwordChangeListener));
  }

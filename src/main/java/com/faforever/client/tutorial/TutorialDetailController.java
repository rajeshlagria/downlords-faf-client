...
public class TutorialDetailController extends AbstractViewController<Node> {
  ...
  public TutorialDetailController(I18n i18n, MapService mapService, WebViewConfigurer webViewConfigurer) {
    this.i18n = i18n;
  ...
  public void setTutorial(Tutorial tutorial) {
    ...
    if (tutorial.getMapVersion() != null) {
      ...
      mapImage.imageProperty().bind(Bindings.createObjectBinding(() -> {
        MapBean mapVersion = tutorial.getMapVersion();
        if (mapVersion.getLargeThumbnailUrl() != null) {
          return mapService.loadPreview(mapVersion, PreviewSize.LARGE);
        } else {
          return IdenticonUtil.createIdenticon(mapVersion.getId());
        }
      }, tutorial.mapVersionProperty(), tutorial.getMapVersion().largeThumbnailUrlProperty()));
